...
public class CamelMethodInvoker {
    ...
    public CamelMethodInvoker(Object instance, Method method) throws Exception {
    ...
    private static class ProcMethodInfo {
        Method method;
        List<ProcParamInfo> parameters = new ArrayList<ProcParamInfo>();
    ...
    private synchronized ProcMethodInfo getMethodInfo(Method method) throws Exception {
    ...
    public List<ProcParamInfo> getParameters(Method method, Class<?>[] parameterTypes) throws Exception {
        Annotation[][] pAnnotations = method.getParameterAnnotations();
        List<ProcParamInfo> parameters = new ArrayList<ProcParamInfo>(pAnnotations.length);
        for (int i = 0; i < parameterTypes.length; ++i) {
            ...
            if (paramInfo == null) {
                throw new MetadataException("Cannot recognize method's '" + method.getName() + "' " + i +
                        " parameter with type " + parameterTypes[i] + "!");
            }
    ...
    private ProcParamInfo getProcParamInfo(Method method, Annotation paramAnnotation, Class<?> paramType) throws Exception {
        if (hasAnnotation(paramAnnotation, Headers.class) || hasAnnotation(paramAnnotation, org.apache.camel.Headers.class)) {
            if (!paramType.isAssignableFrom(Map.class)) {
                throw new MetadataException("Headers argument of type " + paramType + " of processor " + method.getName() +
                        " must be assignable from the body type " + Map.class + " !");
            }
            return new ProcParamInfo(paramType, ProcParamInfo.Kind.HEADERS);
        } else if (hasAnnotation(paramAnnotation, Header.class) || hasAnnotation(paramAnnotation, org.apache.camel.Header.class)) {
            final String name = (String) getAnnotationValue(paramAnnotation, "value");
            ProcParamInfo res = new ProcParamInfo(paramType, ProcParamInfo.Kind.HEADER, name);
            return res;
        } else if (hasAnnotation(paramAnnotation, Body.class) || hasAnnotation(paramAnnotation, org.apache.camel.Body.class)) {
    ...
    private boolean hasAnnotation(Annotation paramAnnotation, Class<? extends Annotation> annotationClass) {
        return paramAnnotation.annotationType().getName().equals(annotationClass.getName());
    }
